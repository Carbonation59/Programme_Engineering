...
namespace database{
    ...
    long Database::count_from_mongo(const std::string &collection)
    {
        try
        {
    ...
    std::vector<std::string> Database::get_from_mongo([[maybe_unused]] const std::string &collection, [[maybe_unused]] std::map<std::string, long> &params_long, [[maybe_unused]] std::map<std::string, std::string> &params_string)
    {
       std::vector<std::string> result;
        ...
        try
        {
            ...
            Poco::MongoDB::QueryRequest request("arch.products");
            Poco::MongoDB::ResponseMessage response;
            if(params_long.size() == 0 && params_string.size() == 0){
                request.selector().addNewDocument("$query");
            } else {
                for (auto &[key, val] : params_long)
                    request.selector().add(key, val);
                for (auto &[key, val] : params_string)
                    request.selector().add(key, val);
            }
            connection_mongo.sendRequest(request, response);

...
namespace database{
    class Database{
        ...
        public:
            ...
            std::vector<std::string> get_from_mongo(const std::string& collection,std::map<std::string,long>& params_long, std::map<std::string,std::string>& params_string);
            long count_from_mongo(const std::string& collection);
    };
}
...
public:
    ProductHandler(const std::string &format, std::vector<database::Product>* products) : _format(format), _products(products)
    {
    }
    ...
    void handleRequest(HTTPServerRequest &request, HTTPServerResponse &response)
    {
        ...
        try
        {
            if (hasPrefix(request.getURI(),"/product") && form.has("id") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            {
                ...
                std::optional<database::Product> result = database::Product::read_by_id(id, _products);
                if (result)
                {
            ...
            else if (hasPrefix(request.getURI(),"/product/search") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET) && form.has("name"))
            {
                ...
                auto results = database::Product::search(name, _products);
                Poco::JSON::Array arr;
                for (auto s : results)
                    arr.add(s.toJSON());
                response.setStatus(Poco::Net::HTTPResponse::HTTP_OK);
            ...
            else if (request.getURI() == "/product" && request.getMethod() == Poco::Net::HTTPRequest::HTTP_PUT)
            {
                if (form.has("name") && form.has("colour") && form.has("weight") && form.has("price") && form.has("id"))
                {
                    ...
                    std::optional<database::Product> result = database::Product::read_by_id(product.get_id(), _products);

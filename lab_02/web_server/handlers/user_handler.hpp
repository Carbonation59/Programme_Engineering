...
public:
    UserHandler(const std::string &format, std::vector<database::User>* users) : _format(format), _users(users)
    {
    }
    ...
    void handleRequest(HTTPServerRequest &request, HTTPServerResponse &response)
    {
        ...
        try
        {
            if (hasPrefix(request.getURI(),"/user") && form.has("id") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            {
                ...
                std::optional<database::User> result = database::User::read_by_id(id, _users);
                if (result)
                {
            ...
            else if (hasPrefix(request.getURI(),"/user") && form.has("login") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            {
                ...
                std::optional<database::User> result = database::User::read_by_login(login, _users);
                if (result)
                {
                    response.setStatus(Poco::Net::HTTPResponse::HTTP_OK);
                    response.setChunkedTransferEncoding(true);
            ...
            else if (request.getURI() == "/user/auth" && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            {
                ...
                if (scheme == "Basic")
                {
                    ...
                    if (auto id = database::User::auth(login, password, _users))
                    {
            ...
            else if (hasPrefix(request.getURI(),"/user/search") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET) && form.has("first_name") && form.has("last_name"))
            {
                ...
                auto results = database::User::search(fn, ln, _users);
                Poco::JSON::Array arr;
                for (auto s : results)
